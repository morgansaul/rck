...
// [ADD] Multi-pubkey container
class PubKeyContainer {
private:
    std::vector<TPubKey> keys;
    CriticalSection cs;
public:
    bool LoadFromFile(const char* path);
    int Count() const { return (int)keys.size(); }
    const TPubKey* Data() const { return keys.data(); }
...
private:
    ...
    PubKeyContainer pubkeys; // [ADD] Multi-pubkey storage
...
public:
    ...
    // [ADD] Pubkey management
    bool LoadPubkeys(const char* path) { return pubkeys.LoadFromFile(path); }
    int PubkeyCount() const { return pubkeys.Count(); }
    const TPubKey* GetPubkeys() const { return pubkeys.Data(); }
};

bool IsFileExist(char* fn);

// [ADD] New functions for pubkey loading
inline bool ParsePubKeyLine(const char* line, TPubKey& key) {
    EcPoint p;
    if (!p.SetHexStr(line)) return false;
    memcpy(key.x, p.x.data, 32);
    memcpy(key.y, p.y.data, 32);
    return true;
}

inline bool PubKeyContainer::LoadFromFile(const char* path) {
